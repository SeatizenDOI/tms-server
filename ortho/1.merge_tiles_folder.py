...
def parse_arg():
    ...
    parser.add_argument("-p", "--path-folder", default="/media/bioeos/E/drone/drone_ortho_tif/output/tiles_uav", help="Path to the folder where your tiles folder are.")
...
def main(opt):
    ...
    for year in sorted(list(years)):
        if year not in ["2023", "2025"]: continue
        global_year = Path(global_tiles_folder, year)
        global_year.mkdir(exist_ok=True, parents=True)

        print(f"Working with {year} \n\n")
        
        # We iterate another time on ASV.
        for folder in list_tiles_folder:
            if folder.name[6:10] != year or "ASV" not in folder.name: continue
            move_tiles(folder, global_year)
        
        # We iterate one time on UAV.
        for folder in list_tiles_folder:
            if folder.name[6:10] != year or "UAV" not in folder.name: continue
            move_tiles(folder, global_year)
